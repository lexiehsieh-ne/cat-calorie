...
import { ENVIRONMENT_LABELS, MAX_FOLLOWUP_QUESTIONS } from "@/lib/calorie-types";
...
export default function CalculatorPage() {
  ...
  const progressPercent = Math.min(100, (questionsAsked / MAX_FOLLOWUP_QUESTIONS) * 100);

  async function callCaloriesApi(nextHistory: CalorieTurn[]) {
    if (!apiKey) {
      setError("請先至設定頁面輸入你的 OpenAI API Key");
      return;
    }
            ...
            <section className="flex flex-col gap-5 rounded-3xl border border-line bg-card p-6 shadow-sm">
              {!apiKey && (
                <div className="flex items-center justify-between gap-3 rounded-xl bg-gold-soft px-4 py-3 text-sm text-foreground">
                  <span>尚未設定 OpenAI API Key，請先完成設定才能開始計算。</span>
                  <Link
                    href="/settings"
            ...
            <section className="flex flex-col gap-4">
              <div className="flex flex-col gap-1.5">
                <div className="flex items-center justify-between text-sm text-foreground/60">
                  <span>
                    已回答 {questionsAsked} 題（最多 {MAX_FOLLOWUP_QUESTIONS} 題）
                  </span>
                </div>
